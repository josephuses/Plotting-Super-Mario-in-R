...
```{r}
a <- str_replace_all(mario, "\n", "")
b <-  str_split(a, "[^[:alnum:]][:blank:]Shape[:blank:][:alnum:][:alnum:]", simplify = TRUE)
c <- data.frame(coords = t(b))
c <- c %>% mutate(shape = rownames(c))
d <- c %>% 
  mutate(coords = strsplit(as.character(coords), " , ")) %>%
  unnest(coords)
d <- d %>% mutate(shape = as.integer(shape) - 1)

# library(splitstackshape)
# e <- lapply(d, function(x) cSplit(x, "x", sep = " , ", direction = "long"))
# d <- lapply(b, function(x) as.data.frame(x))
```
```{r}
e <- separate(d, coords, sep = ",", into = c("x","y"))
paren <- "(|)"
f <- e %>%
  mutate(
